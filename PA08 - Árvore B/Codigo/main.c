...
typedef struct
{
    char chave_primaria[6]; //chave primaria = string concatenada de id_Aluno + sigla_Disciplina
    int rrn;        //rrn do item
} CHAVE_PRIMARIA;
...
//Aloca espaço para um float
NUMERO criar_numero() {
    NUMERO num = (NUMERO) malloc(sizeof(float));
    *num = -1;
    return num;
}
...
REGISTRO criar_registro() {
    REGISTRO reg = (REGISTRO) malloc(sizeof(DADO));
    reg->id_Aluno[0] = '\0';
    reg->sigla_Disciplina[0] = '\0';
    reg->nome_Aluno[0] = '\0';
    reg->nome_Disciplina[0] = '\0';
    reg->media = -1;
    reg->frequencia = -1;
    return reg;
}
...
void limpar_registro(REGISTRO* reg) {
    limpar_string(&(*reg)->id_Aluno);
    limpar_string(&(*reg)->sigla_Disciplina);
    limpar_string(&(*reg)->nome_Aluno);
    limpar_string(&(*reg)->nome_Disciplina);
    limpar_numero(&(*reg)->media);
    limpar_numero(&(*reg)->frequencia);
    free(*reg);
}
...
void add_caractere_string(STRING* str, char caractere) {
    size_t old_len = (*str != NULL) ? strlen(*str) : 0;
    STRING novaString = (STRING) malloc(sizeof(char) * (old_len + 2));
    if (novaString == NULL) {
        // Trate o erro de alocação
        return;
    }

    // Se *str já existir, copie seu conteúdo para novaString
    if (*str != NULL) {
        strcpy(novaString, *str);
    }
    
    novaString[old_len] = caractere;        // Adiciona o novo caractere
    novaString[old_len + 1] = '\0';         // Finaliza a string

    limpar_string(str);                     // Libera a memória antiga
    *str = novaString;                      // Atualiza o ponteiro para a nova string
}
...
PASTA carregar_dados(STRING nomeArquivoInsercao) {
    ...
    while (!fim_de_arquivo) {
        ...
        while(((cont = fread(&caractere,sizeof(char),1,arq)) != 0) && caractere != '\0'){
            add_caractere_string(&(DADO->id_Aluno),caractere);
        }

        while(((cont = fread(&caractere,sizeof(char),1,arq)) != 0) && caractere != '\0'){
            add_caractere_string(&(DADO->sigla_Disciplina),caractere);
        }

        while(((cont = fread(&caractere,sizeof(char),1,arq)) != 0) && caractere != '\0'){
            add_caractere_string(&(DADO->nome_Aluno),caractere);
        }

        while (((cont = fread(&caractere,sizeof(char),1,arq)) != 0) && caractere == '\0') {}

        add_caractere_string(&DADO->nome_Disciplina,caractere);
        while(((cont = fread(&caractere,sizeof(char),1,arq)) != 0) && caractere != '\0'){
            add_caractere_string(&DADO->media,caractere);
        }

        while(((cont = fread(&caractere,sizeof(char),1,arq)) != 0) && caractere != '\0'){
            add_caractere_string(&DADO->frequencia,caractere);
        }
        ...
        *(DADO->media) = atoi(inteiro);
        limpar_string(&inteiro);

        *(DADO->frequencia) = atoi(inteiro);
        limpar_string(&inteiro);
...
PARAGRAFO carregar_chaves(STRING nomeArquivoChaves) {
    ...
    while (!fim_de_arquivo) {
        ...
        id_Aluno = criar_string();
        sigla_Disciplina = criar_string();

        while(((cont = fread(&caractere,sizeof(char),1,arq)) != 0) && caractere != '\0'){
            add_caractere_string(&id_Aluno,caractere);
        }

        while(((cont = fread(&caractere,sizeof(char),1,arq)) != 0) && caractere != '\0'){
            add_caractere_string(&sigla_Disciplina,caractere);
        }
        ...
        add_string_string(&id_Aluno, &sigla_Disciplina);
        add_string_paragrafo(&paragrafo,&id_Aluno);
        limpar_string(&id_Aluno);
        limpar_string(&sigla_Disciplina);
    }
    ...
    do{
        ...
        case 1:
            ...
            if (load_de_arquivos)
            {
                ...
                DADO reg;
                strcpy(reg.id_Aluno, pasta[posRegistro-1]->id_Aluno);
                strcpy(reg.sigla_Disciplina, pasta[posRegistro-1]->sigla_Disciplina);
                strcpy(reg.nome_Aluno, pasta[posRegistro-1]->nome_Aluno);
                strcpy(reg.nome_Disciplina, pasta[posRegistro-1]->nome_Disciplina);
                reg.media = *(pasta[posRegistro-1]->media);
                strcpy(reg.nome_Disciplina, pasta[posRegistro-1]->nome_Disciplina);
                reg.frequencia = *(pasta[posRegistro-1]->frequencia);
...
void inserir_registro(FILE *dataFile, FILE **arvore_b, DADO reg, short *raizRRN)
{
    ...
    chave.rrn = regRRN;
    strcpy(chave.chave_primaria, reg.id_Aluno);
    strcat(chave.chave_primaria, reg.sigla_Disciplina);

    if (adicionar_index(arvore_b, chave, raizRRN) != DUPLICADO){        //adiciona chave do DADO na árvore B
        fwrite(reg.id_Aluno, sizeof(char), sizeof(reg.id_Aluno), dataFile);
        fwrite(reg.sigla_Disciplina, sizeof(char), sizeof(reg.sigla_Disciplina), dataFile);
        fwrite(reg.nome_Aluno, sizeof(char), sizeof(reg.nome_Aluno), dataFile);
        fwrite(reg.nome_Disciplina, sizeof(char), sizeof(reg.nome_Disciplina), dataFile);
        fwrite(&(reg.media), sizeof(float), 1, dataFile);
        fwrite(&(reg.frequencia), sizeof(float), 1, dataFile);
    }
}
